...
2. You will receive an email containing your login details and also confirming operator parameters \<DOMAIN\>, \<PROJECT\>, \<AGENT_TOKEN\> and optionally \<OPTIONAL_NAMESPACE\>
3. The operator parameters can be also found when logged in to Orca: The domain and project are displayed at the top of every Orca screen and the agent token can be found in Settings
4. Run the command: 
```bash
> ./generate_orca_secret.sh <DOMAIN> <PROJECT> <AGENT_TOKEN> <OPTIONAL_NAMESPACE> (if provided) > orca_secret.yaml
